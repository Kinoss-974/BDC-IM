...
export default function FournituresPage() {
  ...
  const categoriesConfig = [
    { name: 'Fourniture sol', icon: <Hammer size={18} /> },
    { name: 'Fourniture mur', icon: <Paintbrush size={18} /> },
    { name: 'Fourniture plafond', icon: <Paintbrush size={18} /> },
    { name: 'Fourniture SDB', icon: <Droplets size={18} /> },
    { name: 'Fourniture Cuisine', icon: <Construction size={18} /> },
    { name: 'Fourniture Électricité', icon: <Lightbulb size={18} /> },
    { name: 'Fourniture Plomberie', icon: <Droplets size={18} /> },
    { name: 'Fourniture Divers', icon: <Package size={18} /> }
  ];
  ...
  const fetchData = async (forceRefresh = false) => {
    setLoading(true);
    try {
      // 1. On récupère le projet (Source : Recap)
      const { data: proj } = await supabase.from('projects').select('*').eq('id', projectId).single();
      setProject(proj);

      if (proj) {
        // 2. On vérifie s'il y a déjà un chiffrage manuel sauvegardé
        ...
        if (savedItems && savedItems.length > 0 && !forceRefresh) {
          ...
          // 3. LA REQUÊTE SYNCHRONISÉE (Grâce au SQL exécuté)
          // Tout s'emboîte : product_type avec product_type, etc.
          const { data: matrix } = await supabase.from('pack_templates')
            .select('*')
            .eq('product_type', proj.product_type)   // Produit (ex: IMMO'MALIN)
            .eq('package_type', proj.package_type)   // Ameublement (ex: LLD)
            .eq('property_type', proj.property_type) // Bien (ex: T2)
            .ilike('category', 'Fourniture%');
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        <div className="xl:col-span-8 space-y-6">
          {categoriesConfig.map((cat) => {
            const catItems = items.filter(i => i.category?.trim().toLowerCase() === cat.name.trim().toLowerCase());
            return (
              <div key={cat.name} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                <div className="bg-slate-50/50 p-6 border-b flex justify-between items-center">
                  ...
                  <button onClick={() => setItems([...items, {id: crypto.randomUUID(), project_id: projectId, category: cat.name, name: '', unit_price: 0, quantity_to_cover: 1}])} className="text-[10px] font-black bg-white border border-slate-200 px-4 py-2 rounded-xl hover:bg-blue-600 hover:text-white transition-all shadow-sm">+ Ajouter</button>
                </div>
                    ...
                    catItems.map((item) => (
                      <div key={item.id} className="group flex flex-wrap md:flex-nowrap items-center gap-4 bg-slate-50/30 hover:bg-white p-4 rounded-2xl border border-transparent hover:border-slate-100 transition-all">
                        ...
                        <div className="flex items-center gap-6">
                          ...
                          <button onClick={() => setItems(items.filter(i => i.id !== item.id))} className="text-slate-200 hover:text-red-500 transition-colors"><Trash2 size={18}/></button>
                        </div>
                      </div>
                    ))
                  )}
