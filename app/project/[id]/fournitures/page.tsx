...
export default function FournituresPage() {
  ...
  // Les sous-dossiers spécifiques aux travaux / fournitures
  // On inclut ceux de ton CSV (sol, SDB, divers) et on anticipe les autres
  const categories = [
    'Fourniture sol', 'Fourniture mur', 'Fourniture plafond',
    'Fourniture SDB', 'Fourniture Cuisine', 'Fourniture Électricité',
    'Fourniture Plomberie', 'Fourniture Menuiserie', 'Fourniture Divers'
  ];
  ...
  const fetchData = async () => {
    ...
    if (proj) {
      ...
      const produit = proj.product_type || "IMMO'MALIN";
      ...
      if (savedItems && savedItems.length > 0) {
        ...
        if (matrix) {
          const initialItems = matrix.map(m => ({
            ...
            category: m.category, // Conserve le sous-dossier (ex: Fourniture sol)
            name: m.name,
            unit_price: 0,
  ...
  const handleSave = async () => {
    ...
    setIsSaving(false);
    alert("Chiffrage Fournitures enregistré !");
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-40">
      ...
      <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white flex flex-wrap gap-8 items-center shadow-xl shadow-slate-200">
        <div className="bg-white/10 p-4 rounded-3xl"><Calculator size={32} /></div>
        <div className="flex-1">
          <h1 className="text-2xl font-black uppercase tracking-tighter italic">Chiffrage Fournitures</h1>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
            ...
            return (
              <div key={cat} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="p-4 space-y-2">
                  {catItems.length === 0 ? (
                    <p className="text-center text-slate-300 italic text-xs py-4">Aucune fourniture dans cette section.</p>
                  ) : (
                    catItems.map((item) => (
                      <div key={item.id} className="flex flex-wrap md:flex-nowrap items-center gap-3 bg-white hover:bg-slate-50 p-3 rounded-2xl border border-transparent hover:border-slate-100 transition-all">
                        <input 
                          placeholder="Désignation (ex: Parquet, Peinture...)"
                          value={item.name} 
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-[9px] font-black text-slate-400 uppercase">Prix HT</span>
                          <input 
                            type="number"
        ...
        <div className="space-y-6">
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm sticky top-28">
            <h3 className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-6">Résumé Fournitures</h3>
